...
void InitSPMSystem(unsigned int numWorkgroups_x, unsigned int numWorkgroups_y, unsigned int numWorkgroups_z)
{
	...
	Vertex* dataArray = new Vertex[mapReader->getNumOfVertices()];
...
void loadDefaultBufferData()
{
	...
	glBufferData(GL_ARRAY_BUFFER, mapReader->getNumOfVertices() * sizeof(Vertex), mapReader->getDataArray(), GL_STATIC_DRAW);
...
void updateSPMSystem(GLuint computeShaderProgram)
{
	...
	glBindBufferRange(GL_SHADER_STORAGE_BUFFER, 0, vertexSSbo[0], 0, mapReader->getNumOfVertices() * sizeof(Vertex));
